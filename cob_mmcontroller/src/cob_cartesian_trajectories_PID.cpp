...
private:
    ...
    ros::Time tstart;
    Frame F_start;
    //std::string mode;     // old variable
    sdt::string model_class;
...
cob_cartesian_trajectories::cob_cartesian_trajectories() : as_(n, "moveCirc", boost::bind(&cob_cartesian_trajectories::moveCircActionCB, this, _1), false), as2_(n, "moveLin", boost::bind(&cob_cartesian_trajectories::moveLinActionCB, this, _1), false)
{
    ...
    serv_model = n.advertiseService("/mm/move_model", &cob_cartesian_trjectories::moveModelCB, this);       // new service to work with models
...
bool cob_cartesian_trajectories::movePriCB()    //TODO // prismatic callback
{
    model_class = "prismatic";
    return start();
}

bool cob_cartesian_trajectories::moveRotCB()    //TODO // rotational callback
{
    model_class = "rotational";
    return start();
}
...
{
    if (request->model.name == "prismatic" || request->model.name == "rotational")
    {
        model_class = request->model.name;
        return start(request->target_duration.toSec());     // forward params and duration ???
    }
    else
    {
        response->success = false;
        response->error_message = "Unkown model class";
        ROS_ERROR("Unkown model class");
        return false;
    }
}

bool cob_cartesian_trajectories::start(cob_mmcontroller::MoveModel::Request& request) //TODO request->model.params // start 
{
    if(bRun)
    {
