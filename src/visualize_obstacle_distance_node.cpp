...
public:
    ...
    void obstacleDistancesCallback(const obstacle_distance::DistanceInfos::ConstPtr& msg)
    {
        ...
        std::map<std::string, obstacle_distance::DistanceInfo> relevant_obstacle_distances;
        ...
        for(uint32_t i=0; i<msg->infos.size(); i++)
        {
            ...
            if(relevant_obstacle_distances.count(id) > 0)
            {
                ...
                {
                    relevant_obstacle_distances[id] = msg->infos[i];
                }
            }
            else
            {
        ...
        for(std::map<std::string, obstacle_distance::DistanceInfo>::const_iterator it = relevant_obstacle_distances.begin();
                it != relevant_obstacle_distances.end(); ++it)
        {
            ...
            Eigen::Vector3d obst_np(it->second.nearest_point_obstacle_vector.vector.x,
                                    it->second.nearest_point_obstacle_vector.vector.y,
                                    it->second.nearest_point_obstacle_vector.vector.z);
            Eigen::Vector3d frame_np(it->second.nearest_point_frame_vector.vector.x,
                                     it->second.nearest_point_frame_vector.vector.y,
                                     it->second.nearest_point_frame_vector.vector.z);

            // Transform the coordinate systems
            Eigen::Vector3d obst_bl_np = transform(marker_vector.header.frame_id , it->second.nearest_point_obstacle_vector.header.frame_id) * obst_np;
            Eigen::Vector3d frame_bl_np = transform(marker_vector.header.frame_id , it->second.nearest_point_frame_vector.header.frame_id) * frame_np;
